...
describe("profile API route", () => {
  ...
  it("updates only the current user's profile with parsed input", async () => {
    const input = { children: [{ birthYearMonth: "2024-09" }] };
    mocks.requireCurrentUser.mockResolvedValue({ id: "user-1" });
    mocks.schemaParse.mockReturnValue(input);
    mocks.updateMyProfile.mockResolvedValue({ children: [{ id: "child-1", birthYearMonth: "2024-09", sortOrder: 0 }] });
    ...
    await expect(response.json()).resolves.toMatchObject({
      children: [{ id: "child-1", birthYearMonth: "2024-09" }]
    });
  });
});
