...
describe("result score labels", () => {
  it("keeps visible score labels compact and distinct", () => {
    expect(searchRelevanceScoreLabel(56)).toBe("검색 56");
    expect(placeQualityScoreLabel(88)).toBe("장소 88");
  });
  ...
  it("describes what each score means for assistive labels and tooltips", () => {
    ...
    expect(placeQualityScoreTitle(88)).toBe("장소 자체 품질 점수 88점");
    expect(resultScoreRowLabel(56, 88)).toBe("현재 검색 조건 기준 관련도 56점, 장소 자체 품질 점수 88점");
  });
