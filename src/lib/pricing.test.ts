...
describe("pricing helpers", () => {
  ...
  it("falls back to the first structured item and flags missing verification dates", () => {
    ...
    expect(pricingItemLabels(pricing)).toEqual(["보호자 입장: 4,000원 / 보호자"]);
    expect(pricingSummaryLabel(pricing)).toBe("보호자 입장: 4,000원 / 보호자 · 확인일 미등록");
  });
  ...
  it("formats non-KRW local currency amounts", () => {
    ...
    expect(pricingItemLabels(pricing)).toEqual(["데이패스: PHP 2,500 / 1인", "어린이 입장: US$1,200"]);
  });
});
