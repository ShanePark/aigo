...
User-facing public place text must be written in Korean. API field names and structured keys can stay English/camelCase, but visible public text such as `description`, `parentNotes`, `safetyNotes`, `placeScoreRationale`, `playFeatures.notes`, pricing summaries/items, opening-hours notes, route-support notes, and image alt/description text should not contain English template prose, English price prose, or agent shorthand. Translate source-backed facts into natural Korean before registration or enrichment; keep English only for proper nouns, URLs, source titles, currency codes, structured keys, and unavoidable facility names.
...
- Keep default surfaces simple and concise. Avoid verbose helper text, value-prop prose, and explanatory sentences in login screens, cards, panels, and headers; preserve only labels/status needed for the next action, and move optional detail behind an info icon, disclosure, or modal.
