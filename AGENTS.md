...
- Family accommodations: keep the top-level category as `accommodation`, and classify the user-facing lodging subtype with source-backed tags. The active subtype set is `resort`, `pool_villa`, `kids_hotel`, and `pension`; do not use broad "키즈 숙소" as a category or subtype when sources can distinguish the lodging model.
