...
- [대기] AiGo Facet Taxonomy V1 Phase 2부터 순차 구현한다. 작업자는 먼저 `docs/aigo-facet-taxonomy-v1.md`를 읽고 그 문서의 phase 순서와 queue guardrails를 따른다. Phase 1 카탈로그/입력 검증은 완료되었으므로 다음 작업은 `places.taxonomy` JSONB, GIN index, derived search text, preflight schema, API 응답/저장 wiring, source replace 이미지 연결 보존을 포함하는 Phase 2부터 진행한다. 병렬 하트비트 작업과 충돌하기 쉬우므로 시작 전과 각 phase 후 `git status --short`를 확인하고, 활성 편집 중인 파일이 있으면 병렬 구현하지 말고 큐에서 순차 진행한다. Source: user taxonomy planning discussion on 2026-05-22.
