...
방문/방문로그 UI 작업은 모바일을 1급 기준으로 둔다. 구현자는 데스크톱에서만 맞춰 놓고 끝내지 말고, Playwright로 iPhone 13, iPhone 14/15 계열, iPhone 16 Pro, 작은 Android 폭(360px대), 넓은 Android 폭(412px대) 같은 흔한 모바일 해상도를 실제로 열어 스크린샷을 확인하며 수정한다. 각 해상도에서 첫 화면, 스크롤 중간, 입력/업로드/비로그인/빈 상태, 긴 장소명과 긴 리뷰, 공개/비공개/재방문 배지가 모두 겹치지 않는지 눈으로 본다. 좋은 모바일 제품의 참고점은 Airbnb처럼 카드의 정보 위계가 명확하고 터치 타깃이 넉넉하며, Google Maps/Apple Maps처럼 장소 맥락과 액션이 빠르게 읽히고, Instagram/네이버 지도처럼 사진·리뷰·저장/공유 성격의 조작이 손가락 흐름 안에 자연스럽게 놓이는 것이다. 단순히 흉내 내기보다 AiGo의 가족 외출 맥락에 맞게 정보 밀도, 여백, 고정/복귀 액션, 하단 CTA, 아이콘 레이블, 손쉬운 재방문 탐색을 조정한다. 구현 완료 전에는 `@media` 규칙과 실제 Playwright 스크린샷을 함께 확인하고, 어색한 장면이 하나라도 있으면 해당 항목을 끝내지 않는다.

구현 체크리스트:

- [대기] 장소 상세의 `방문했어요` 입력 UI/UX를 실제 터치·클릭 흐름 기준으로 다시 설계한다. 현재 평점 선택은 숫자와 별 버튼이 작고 클릭 피드백이 약해 모바일에서 누르기 어색하며, 공개 범위 `공개/비공개` 토글과 사진 등록 input도 폼 요소 느낌이 강해 방문 기록을 남기는 감각이 딱딱하다. 별점은 큰 별 아이콘 기반의 단일 선택 컨트롤로 바꾸고 hover/focus/selected/disabled 상태를 명확히 하며, 공개 범위는 `Globe2`/`Lock` 같은 아이콘 중심의 compact segmented control 또는 icon chip으로 정리한다. 사진 등록은 파일 input 원형 노출 대신 카메라 아이콘, 선택된 파일명/제거 버튼, 업로드 가능 형식 안내, 저장 중 상태가 있는 업로드 카드로 만든다. 모바일에서는 한 손으로 별점 선택, 공개범위 전환, 사진 추가, 저장 CTA까지 이어지는 흐름을 우선 설계하고, 긴 장소명·긴 리뷰 placeholder·키보드가 열린 상태에서도 버튼과 상태 문구가 가려지지 않게 한다. Airbnb의 모바일 카드/예약 흐름처럼 핵심 정보와 액션을 명확히 분리하고, Google Maps/Apple Maps의 장소 패널처럼 장소 맥락과 사용자 액션이 한눈에 이어지도록 참고한다. 관련 코드: `src/app/places/place-visit-panel.tsx`, `src/app/globals.css`. 검증: 로그인 상태에서 별점 1-5점 선택, 공개/비공개 전환, 사진 선택/취소, 저장 중 비활성화가 desktop/mobile에서 어색하지 않은지 Playwright 스크린샷으로 확인한다. 최소 iPhone 13(390x844), iPhone 16 Pro 계열(402x874 또는 Playwright device preset), 360x800, 412x915에서 패널 상단·폼·사진 선택 후·저장 오류/성공 상태를 캡처하고, 접근성 이름과 키보드 포커스가 유지되는지 점검한다.
...
- [대기] `/visits` 방문로그 화면을 더미데이터 기반으로 먼저 눈으로 검토하며 전면 UI/UX 개선 계획을 실행한다. 현재 방문로그는 기본 카드와 요약 수치가 있으나 메인으로 돌아가는 명확한 버튼이 부족하고, 실제 데이터가 적거나 없을 때 화면의 완성도가 떨어질 수 있다. 구현 전 로컬 UI 검증용 더미 방문로그 몇 건을 준비한다. 예시는 같은 날짜 2곳, 다른 날짜 1곳 이상, 공개/비공개, 재방문, 사진 있음/없음, 리뷰 긴 문장/짧은 문장/없음, 서로 다른 카테고리를 포함하되 real seed/export로 만들지 말고 테스트 fixture, Storybook류 격리 파일, 또는 개발 전용 렌더 helper처럼 운영 데이터와 분리한다. 그 더미 상태를 기준으로 상단에 메인/장소 찾기로 돌아가는 아이콘 버튼을 추가하고, 요약 영역, 날짜 그룹, 카드 정보 밀도, 별점·사진·공개범위 아이콘, 빈 상태, 비로그인 상태를 한 화면 흐름으로 재정리한다. 모바일에서는 날짜별 타임라인, 카드 터치 영역, 별점/사진/공개범위 메타 정보, 긴 리뷰 접힘/줄수 제한, 상단 복귀 액션 또는 하단 고정 CTA가 엄지손가락 동선에 맞는지 우선 본다. Airbnb의 여행/위시리스트 모바일 화면처럼 카드 간 리듬과 이미지·텍스트 위계를 참고하고, Google Maps/네이버 지도처럼 장소 목록에서 빠르게 스캔 가능한 메타 배치를 참고한다. 관련 코드: `src/app/visits/page.tsx`, `src/app/topbar-actions.tsx`, `src/app/globals.css`, 필요 시 `src/lib/place-visits.test.ts` fixture. 검증: 더미데이터 화면, 실제 빈 로그 화면, 비로그인 화면을 desktop과 iPhone 13(390x844), iPhone 16 Pro 계열(402x874), 360x800, 412x915에서 Playwright로 각각 캡처해 비교한다. 첫 화면에 목적과 복귀 액션이 보이는지, 스크롤 중 날짜 헤더와 카드가 답답하지 않은지, 카드 클릭이 장소 상세로 이동하며 `returnTo=/visits`로 돌아오는 동선과 새 메인 복귀 버튼이 모두 동작하는지 확인한다. 스크린샷에서 텍스트 겹침, 배지 줄바꿈 깨짐, CTA 과밀, 과도한 카드 중첩이 보이면 수정 후 같은 해상도 세트를 다시 캡처한다.
