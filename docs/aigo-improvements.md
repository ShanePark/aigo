...
- [대기] Place-data automation credential preflight and cooldown: `agent-research/aigo-collection-context.md` had accumulated 31 repeated Jeonbuk credential-blocker follow-ups after local logic incorrectly treated the user-confirmed real `.env` key value as a placeholder. Add or document a durable preflight path that checks missing/empty credentials and production 401/403 responses, not hard-coded literal key strings, before spawning subagents, production read-only API artifacts, count movement, or repeated blocker context writes; unchanged auth blockers should end quietly instead of notifying every heartbeat. Source audit: `agent-research/cleanup-status-20260616.md`.
