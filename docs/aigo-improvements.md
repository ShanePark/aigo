...
- [대기] source-backed 이미지 추출/렌더링 검증 헬퍼를 만든다. Wave30 route-break와 accommodation 리서치에서 KAC 대구공항 페이지는 유아휴게실 이미지가 `data:image/png;base64`로만 노출되어 API 이미지 요건을 통과하지 못했고, 한국도로공사 휴게소 공공데이터는 편의시설 근거는 있으나 장소 이미지가 없었으며, Sono/Lahan/리조트 공식 페이지는 API형 이미지 URL, referer 요구, 느린 `fileImage.do` 응답 등으로 수동 검증이 반복됐다. 헬퍼는 후보 URL, sourceUrl, title, status/content-type/content-length, referer 필요 여부, hotlink 위험, 로고/제네릭 여부를 출력해 place registration 전 image-health 실패를 줄여야 한다.

- [대기] `/v1/places/search` 응답 shape를 agents가 오독하지 않도록 예제와 공용 read-only search helper를 추가한다. Wave30 kid-primary accommodations 리서치(`agent-research/wave30-kid-primary-accommodations-resorts-20260523-0110.md`)에서 스크립트가 과거/추정 응답인 `results` 배열을 찾다가 현재 top-level `items` 응답을 놓쳐 false zero count가 날 뻔했다. skill/OpenAPI agent notes에 compact/exact-name 예제 응답을 넣고, research scripts가 `items`/legacy alias를 안전하게 읽는 helper를 제공한다.
