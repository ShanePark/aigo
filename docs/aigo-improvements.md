...
- [대기] **최우선** 지도 검색 UX를 "항상 현재 지도 화면 기준"으로 재정의한다. 세부 조건에서 사용자가 직접 입력하는 위도, 경도, 반경 km 조건은 제거하고, 검색 범위는 항상 지도 컴포넌트의 현재 viewport bounds(`minLat`, `minLng`, `maxLat`, `maxLng`)가 되어야 한다. 지도 이동/확대/축소 후 `현 지도에서 검색`을 누르면 고정 반경 20km/80km나 수동 좌표가 아니라 현재 보이는 지도 사각 영역 안에 포함된 모든 장소를 `/v1/places/search`의 `viewportBounds`로 조회한다. `origin`은 거리 라벨과 추천 점수 계산에만 쓰고, viewport 검색에서는 `filterByRadius: false`를 사용한다. 버튼 클릭 시 지도 viewport, zoom, center, 컨테이너 높이/너비는 그대로 유지하고 목록 데이터와 지도 마커만 비동기로 교체되어야 하며, URL/search params 갱신이나 서버 컴포넌트 재조회 때문에 지도 컴포넌트가 unmount/remount되지 않게 한다. 로딩 중에는 지도 전체 skeleton이나 빈 상태로 갈아끼우지 말고 기존 지도 위에 작은 진행 상태만 표시한다. Playwright로 세부 조건에 위도/경도/반경 입력이 보이지 않는지, `현 지도에서 검색` 클릭 전후 map container bounding box와 center/zoom이 유지되는지, 스크린샷 diff에서 흰 화면 깜빡임이 없는지 검증한다.
...
- [대기] 키즈카페와 유료 실내놀이시설의 가격, 회차, 예약, 당일 이용 가능성, 대표 이미지 데이터를 우선 보강하고 검색에서 이 누락을 강하게 드러낸다. 2026-05-23 키즈카페 검색에서 코코몽키즈랜드 대전중앙로역점과 타요키즈카페 대전둔산점이 추천권에 있었지만 예약/워크인/회차/당일 가능 여부가 공통으로 비어 있었고, 타요키즈카페는 `imageHealth=no_active_image`였다. 가격 정보는 별도 엔티티 또는 구조화 필드로 관리하되 항상 `priceCheckedAt` 또는 `priceBasisDate`를 노출한다. 추천 UI/API에는 `가격 확인 필요`, `회차 확인 필요`, `대표 이미지 없음` 같은 agent-facing caution을 내려준다.
