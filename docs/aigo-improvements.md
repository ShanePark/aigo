...
- [대기] 구조화 이미지 append에서 기존 대표 이미지를 의도치 않게 바꾸지 않도록 primary 제어를 분리한다. Wave31 서울상상나라 보강 중 `PATCH /v1/places/d5e0d90a-4ad3-4aa6-bba8-ae28eafda4d4`에 `imageMode: "append"`와 `images: [{ url: "https://www.seoulchildrensmuseum.org/z00_images/reservation/convenience_img3.png", isPrimary: false, sortOrder: 1, ... }]`를 보냈지만, `normalizeImageInputs()`가 요청 내 첫 active 이미지를 primary로 승격하고 conflict primary overwrite가 켜져 기존 KTO 대표 이미지가 비대표가 되었다. 별도 `imageMode: "replace"` 패치로 복구했지만, API는 기존 primary가 있는 append에서는 `isPrimary: false`를 존중하거나 `primaryOverwrite`/`appendAsSecondary` 같은 명시 옵션을 제공해야 한다. 기존 primary가 있는 place에 비대표 보조 이미지를 append하는 회귀 테스트를 추가한다.
