...
- [대기] 검색 홈의 첫 화면 정보 위계를 정리한다. 범위: `src/app/page.tsx`의 검색 폼, 카테고리 탭, 결과 헤더와 `src/app/globals.css`의 상단 레이아웃. 개선 내용: 검색어 입력과 검색 버튼을 가장 강한 행동으로 보이게 하고, 카테고리 탭은 아이콘과 짧은 라벨 중심으로 정리하며, 결과 수/현재 위치/정렬 상태가 한눈에 보이게 한다. 완료 기준: 데스크톱과 모바일 첫 화면에서 검색, 카테고리, 결과 요약이 서로 겹치지 않고 44px 이상 터치 타깃을 유지한다. 검증: `pnpm lint`, 가능하면 Playwright 또는 브라우저 스크린샷으로 390px/768px/1440px 확인.
- [대기] 지도와 리스트의 분할 탐색 경험을 다듬는다. 범위: `src/app/places-map.tsx`, `src/app/page.tsx`의 `explore-layout`, `results-panel`, `results-scroll` 스타일. 개선 내용: 데스크톱에서는 지도와 리스트가 같은 작업면처럼 연결되게 높이와 여백을 조정하고, 모바일에서는 지도와 결과가 과도하게 서로 밀어내지 않게 순서와 접힘/높이 정책을 정한다. 완료 기준: 지도 marker hover/click과 장소 카드 hover/focus가 시각적으로 연결되고, 리스트 스크롤과 페이지 스크롤이 어색하게 중첩되지 않는다. 검증: `pnpm lint`, 브라우저에서 지도 로딩/스크롤/카드 이동을 데스크톱과 모바일로 확인.
