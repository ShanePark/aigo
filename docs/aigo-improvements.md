...
- [대기] 방문 사진 공개범위를 부모 방문 기록 공개범위와 일관되게 강제한다. 현재 `POST /api/visits/[visitId]/photos`는 사진 공개범위를 별도로 받아 `place_visit_photos.visibility`에 저장하고, `PATCH /api/visits/[visitId]`는 방문 기록의 `visibility`만 바꾼다. 그래서 공개 방문에 올린 사진이 이후 방문 기록을 비공개로 바꿔도 `/api/visit-photos/[photoId]`에서 계속 공개될 수 있고, 비공개 방문에 public 사진을 붙이는 API 호출도 막지 못한다. 업로드 시 부모 방문이 private이면 사진도 private으로 강제하거나 거부하고, 방문 공개범위 변경 시 기존 사진을 함께 숨기거나 스트리밍/요약 쿼리에서 부모 방문 공개범위를 함께 검증하도록 정리한다. 관련 코드: `src/app/api/visits/[visitId]/photos/route.ts`, `src/lib/visit-photos.ts`, `src/lib/place-visits.ts`. 검증: private 방문에 public 사진 업로드 시나리오, public 사진 업로드 후 방문 private 전환 시나리오, 익명 사진 스트리밍 차단 테스트를 추가한다.
- [대기] dev 로그인/로그아웃 후 서버 렌더링 화면과 클라이언트 인증 상태를 즉시 일치시킨다. 현재 topbar의 `DevAuthControls`는 로그인/로그아웃 후 커스텀 이벤트로 클라이언트 패널만 갱신하고 서버 컴포넌트 페이지는 refresh하지 않아, `/visits`처럼 쿠키로 렌더링된 화면이 다음 네비게이션 전까지 이전 인증 상태를 보여줄 수 있다. 또한 logout 요청의 `response.ok`를 확인하지 않고 UI를 로그아웃 상태로 바꿔, 서버 쿠키 삭제가 실패했을 때 UI와 세션이 어긋날 수 있다. 로그인/로그아웃 성공 처리에서 `router.refresh()` 또는 명확한 재검증 경로를 사용하고, 실패 응답은 사용자에게 표시하며 로컬 상태를 무조건 변경하지 않도록 한다. 관련 코드: `src/app/dev-auth-controls.tsx`, `src/app/visits/page.tsx`, `src/app/api/auth/logout/route.ts`. 검증: logout 실패 mock, 로그인 직후 방문 로그 접근, 로그아웃 직후 방문 로그 비노출 흐름을 테스트한다.
