...
- [대기] 지도 화면의 `현 지도에서 검색` 동작에서 지도 크기 변화와 깜빡임을 없앤다. 버튼을 누르면 현재 지도 viewport, zoom, center, 컨테이너 높이/너비는 그대로 유지하고 목록 데이터와 지도 마커만 비동기로 교체되어야 한다. 구현 시 URL/search params 갱신이나 서버 컴포넌트 재조회 때문에 지도 컴포넌트가 unmount/remount되지 않는지 확인하고, map container는 고정된 responsive height를 유지한다. 로딩 중에는 지도 전체 skeleton이나 빈 상태로 갈아끼우지 말고 기존 지도 위에 작은 진행 상태만 표시한다. Playwright로 `현 지도에서 검색` 클릭 전후 map container bounding box, center/zoom, 스크린샷 diff를 확인해 크기 변화와 흰 화면 깜빡임이 없는지 검증한다.
