...
- [대기] AiGo Facet Taxonomy V1 Phase 5 문서화 검수를 마무리한다. 작업자는 먼저 `docs/aigo-facet-taxonomy-v1.md`의 Phase 5 요구사항과 queue guardrails를 확인하고, Phase 1 카탈로그/입력 검증, Phase 2 `places.taxonomy` DB/API wiring, Phase 3 taxonomy 검색 soft/required mode 및 reason code, Phase 4 `scripts/audit-taxonomy.ts` 및 dry-run 기본 `scripts/apply-taxonomy-migration.ts`가 이미 구현되어 있다는 전제에서 진행한다. `docs/openapi/aigo-v1.yaml`, `.codex/skills/aigo-place-api/SKILL.md`, `README.md`가 현재 taxonomy API 계약, canonical primary/source/region 규칙, audit/apply 스크립트 사용법, 검증 명령과 불일치하지 않는지 확인하고 필요한 최소 문서 수정만 한다. 문서만 바뀌는 단계이므로 실제 place data mutation은 하지 말고, 완료 후에는 이 항목을 제거한다. Source: user taxonomy planning discussion on 2026-05-22.
