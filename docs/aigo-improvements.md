...
- [대기] AiGo Facet Taxonomy V1 Phase 4부터 순차 구현한다. 작업자는 먼저 `docs/aigo-facet-taxonomy-v1.md`를 읽고 그 문서의 phase 순서와 queue guardrails를 따른다. Phase 1 카탈로그/입력 검증, Phase 2 `places.taxonomy` DB/API wiring, Phase 3 taxonomy 검색 soft/required mode 및 reason code는 완료되었으므로, 다음 작업은 read-only `scripts/audit-taxonomy.ts`와 dry-run 기본 `scripts/apply-taxonomy-migration.ts`를 추가하는 Phase 4부터 진행한다. apply 스크립트는 실제 place data를 직접 DB update하지 말고 API PATCH만 사용해야 하며, `--apply`가 없으면 mutation 없이 요약만 출력해야 한다. 병렬 하트비트 작업과 충돌하기 쉬우므로 시작 전과 각 phase 후 `git status --short`를 확인하고, 활성 편집 중인 파일이 있으면 병렬 구현하지 말고 큐에서 순차 진행한다. Source: user taxonomy planning discussion on 2026-05-22.
