...
- [대기] AiGo Facet Taxonomy V1 Phase 3부터 순차 구현한다. 작업자는 먼저 `docs/aigo-facet-taxonomy-v1.md`를 읽고 그 문서의 phase 순서와 queue guardrails를 따른다. Phase 1 카탈로그/입력 검증과 Phase 2 `places.taxonomy` DB/API wiring은 완료되었으므로, 다음 작업은 검색 요청의 `taxonomy` soft/required mode, SQL JSONB containment required filters, query-inferred taxonomy 병합, taxonomy scoring/reason code(`TAXONOMY_ACTIVITY_MATCH`, `TAXONOMY_USE_CASE_MATCH`, `TAXONOMY_LOGISTICS_MATCH`, `TAXONOMY_RISK_FLAG`, `TAXONOMY_UNKNOWN`) 추가부터 진행한다. 병렬 하트비트 작업과 충돌하기 쉬우므로 시작 전과 각 phase 후 `git status --short`를 확인하고, 활성 편집 중인 파일이 있으면 병렬 구현하지 말고 큐에서 순차 진행한다. Source: user taxonomy planning discussion on 2026-05-22.
