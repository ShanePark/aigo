...
7. Verify after meaningful changes.
   ...
   - For duplicate-review calls that only need identity and safety signals, send `projection: "compact"` to `/v1/places/duplicates`; compact duplicate candidates keep nested `place.id/name/category/location/status`, `confidence`, `suggestedAction`, `relationshipHint`, `distanceMeters`, `nameSimilarity`, `outsideRadiusReviewOnly`, and `reasonCodes` while omitting full detail images, sources, versions, play features, and scoring payloads.
