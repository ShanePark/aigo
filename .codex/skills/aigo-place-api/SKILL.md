...
3. Run shallow discovery before deep research.
   ...
   - When an official Daum/Kakao roughmap source exposes `WCONGNAMUL` coordinates such as `placeX/placeY` or `markerData.x/y`, run `pnpm tsx scripts/convert-roughmap-coordinate.ts --url=<roughmapJsonUrl> --json` or pass `--x=<x> --y=<y>` instead of using ad hoc conversion snippets. Copy the helper's WGS84 output and coordinate provenance basis into the research handoff or API payload draft.
