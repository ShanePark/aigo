...
5. Check duplicates before mutation.
   ...
   - Treat `PUBLIC_PROVIDER_SIBLING_REVIEW_ONLY` as a same-provider public-facility sibling caution, not a blocking duplicate. Different branches of one 육아종합지원센터/가족센터/도서관 service, such as separate `장난감도서관` branches, can share provider and service names; use exact-name lookup and branch-specific address/source evidence before patching.
...
7. Verify after meaningful changes.
   ...
   - When same-provider public child-care branches such as `공동육아나눔터`, `공동육아방`, or toy-library branches return `PUBLIC_PROVIDER_SIBLING_REVIEW_ONLY` with `reviewBucket: "sibling_branch_review"`, treat them as distinct sibling-branch review leads unless strict identity evidence also matches. Branch labels such as `1호점`, named branches like `별빛`, or district/place-name anchors like `용진` and `삼봉` should prevent a generic same-region alias match from becoming a hard duplicate hold on its own.
