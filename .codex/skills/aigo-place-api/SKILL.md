...
6. Verify after meaningful changes.
   ...
   - Search response meta includes `search.preferenceSemantics` to make clear whether facility preferences are soft ranking signals or required filters. By default, unknown or mismatched records can still appear with reason codes explaining the tradeoff; send `preferenceMode: "required"` when stroller, diaper, nursing, parking, baby-chair, food, or indoor-type requirements should exclude unknown/mismatched records.
...
- Facility preferences default to soft scoring; use search `preferenceMode: "required"` when the planning context needs hard logistics gates, such as diaper table or nursing room evidence for twin-infant outings.
