...
7. Verify after meaningful changes.
   ...
   - Search and detail responses include `openingHoursSummary` so source-backed operating-hours, reservation, walk-in, session, and temporary-closure evidence remains visible even when runtime scoring cannot evaluate `OPEN_NOW` from normal weekly hours. `openingHours.temporaryClosure` is first-class structured evidence and appears as `openingHoursSummary.temporaryClosure`; during the active closure period, search scoring should treat it as `CLOSED_NOW`, not `OPENING_HOURS_UNKNOWN`.
