...
7. Verify after meaningful changes.
   ...
   - When stale or broken direct image URLs are suspected, use scoped direct probing: `GET /v1/places/image-health?status=attention&placeIds=<id1>,<id2>&probeImages=true`. This is intentionally opt-in, requires 25 or fewer scoped `placeIds`, uses HEAD followed by ranged GET, and may return `imageHealth.status: "primary_image_unreachable"` with `directProbe` details. Do not use direct probing for broad unscoped queue scans.
