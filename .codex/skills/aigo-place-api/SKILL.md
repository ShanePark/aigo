...
- Write user-facing public place text in Korean. API field names, taxonomy keys, and internal notes may stay English/camelCase, but `description`, `parentNotes`, `safetyNotes`, `placeScoreRationale`, and `playFeatures.notes` must read naturally for Korean AiGo users. Do not leave English template prose such as "source-backed candidate", "family fallback", or "infant logistics" in public fields; translate it into Korean before mutation. English proper nouns, brand names, source titles, URLs, and short unavoidable facility names are acceptable when surrounded by Korean explanation.
...
4. Research with source notes.
   ...
   - Before staging or mutating a payload, run a Korean-public-text pass over `description`, `parentNotes`, `safetyNotes`, `placeScoreRationale`, and `playFeatures.notes`. If those fields contain English summary sentences or agent shorthand, rewrite them in Korean using the same source-backed facts. Keep English only for proper nouns, URLs, source titles, and unavoidable facility names.
