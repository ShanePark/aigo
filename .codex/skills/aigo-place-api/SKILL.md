...
- Write all user-facing public place text in Korean for Korean AiGo users. API field names, taxonomy keys, source titles, URLs, and structured enum/camelCase keys may stay English, but visible text values must read naturally in Korean before mutation. This includes `description`, `parentNotes`, `safetyNotes`, `placeScoreRationale`, `playFeatures.notes`, `pricing.summary`, `pricing.notes`, `pricing.items[].label`, `pricing.items[].unit`, `pricing.items[].ageRange`, `pricing.items[].conditions`, `pricing.items[].notes`, `openingHours` human-readable summaries/notes, `routeSupport.notes`, `routeSupport.babyCareLocations[].label`, `routeSupport.babyCareLocations[].directions`, `routeSupport.strollerRental.notes`, `images[].altText`, and `images[].description`. Do not leave English template prose such as "source-backed candidate", "family fallback", "infant logistics", "official page confirms", or English price summaries in public fields; translate source-backed facts into Korean before mutation. English proper nouns, brand names, source titles, URLs, currency codes, and unavoidable facility names are acceptable when surrounded by Korean explanation.
...
- During place research, data collection, or API registration waves, do not stop to fix product/API/schema/tooling issues discovered along the way. Add an actionable `[대기]` proposal to `docs/aigo-improvements.md` with enough context for the separate improvement automation to pick it up later. Before implementing a queued improvement later, review it while it is still `[대기]` and confirm it is still real, necessary, worth doing, and unlikely to introduce product, data, API, migration, UX, or operational problems.
...
- Capture source-backed facility scale, free or low-cost public value, provider ratings, review-count confidence, source conflicts, freshness, and evidence caps in `scoreSignals` when useful. The runtime scorer gives a small `externalEvidence` bonus for `scoreSignals.facilityScale`, `scoreSignals.freeAdmission`, `scoreSignals.lowCost`, source-backed low-cost `pricing`, and explicit `pricing.freeAdmission: true`; keep these signals conservative and source-backed. Do not encode bundled or lodging-included amenities as place-level free admission.
...
3. Run shallow discovery before deep research.
   ...
   - When a regional slice has candidate names spread across several old research files or waves, run `pnpm tsx scripts/audit-region-candidates.ts --region=<region> --candidate=<name> --candidate=<name>` before deep research. The helper performs read-only exact-name lookup, duplicate checks, detail/version reads, image-health lookup, and source freshness summaries so agents can separate registered, update-ready, duplicate-review, and missing candidates without reopening every source page. It treats search items with `QUERY_NAME_EXACT` as strict exact matches and reports broader exact-name search recall under `relatedExactMatches`; use related items as context/noise, not proof that the candidate is already registered.
   - `scripts/audit-region-candidates.ts` defaults to the production AiGo API (`https://aigo.o-r.kr`) for place-data audits and prints the resolved read-only base URL during healthcheck; pass `--api-base-url=http://localhost:3000` only for explicit disposable local route tests.
   - When exact-name records already exist and the research value is enrichment rather than creation, run `pnpm tsx scripts/extract-update-ready-patches.ts --candidate=<existing name> --candidate=<existing name>`. The helper defaults to the production AiGo API (`https://aigo.o-r.kr`) like `scripts/audit-region-candidates.ts`; pass `--api-base-url=http://localhost:3000` only for explicit disposable local route tests. It reads place detail, source freshness, latest version summary, image coverage, and missing family/logistics fields, then prints a PATCH handoff skeleton with field-level instructions. Treat the skeleton as a source-backed worklist, not as an API-ready payload until TODO source values are filled.
   ...
   - When an official Daum/Kakao roughmap source exposes `WCONGNAMUL` coordinates such as `placeX/placeY` or `markerData.x/y`, run `pnpm tsx scripts/convert-roughmap-coordinate.ts --url=<roughmapJsonUrl> --json` or pass `--x=<x> --y=<y>` instead of using ad hoc conversion snippets. When an official/public-agency page exposes a projected Korea 2000 map point such as `map1._showpoint(x, y, zoom)`, run `pnpm tsx scripts/convert-official-map-coordinate.ts --x=<x> --y=<y> --crs=EPSG:5179 --source-url=<sourceUrl> --label=<matched address> --json`. Copy the helper's WGS84 output and coordinate provenance basis into the research handoff or API payload draft.
...
4. Research with source notes.
   ...
   - Before staging or mutating a payload, run a Korean-public-text pass over every user-facing public text value, including place notes, scoring rationale, pricing, opening-hours notes, route-support notes, play-feature notes, and image alt/description text. If those fields contain English summary sentences, English price prose, or agent shorthand, rewrite them in Korean using the same source-backed facts. Keep English only for proper nouns, URLs, source titles, currency codes, structured keys, and unavoidable facility names.
   ...
   - When a place appears unusually broad, free/very-low-cost, or multi-use, capture facility scale and free-entry evidence explicitly. Use `pricing.freeAdmission: true` only for source-backed place-level free admission, not for bundled hotel/resort amenities or free subprograms. Use `pricing` for source-backed fees, `scoreSignals.facilityScale` / `scoreSignals.freeAdmission` for scoring evidence, and `placeScoreRationale` to explain why the scale or cost changes family value. If the current runtime scorer does not yet use that signal, add a `[대기]` item to `docs/aigo-improvements.md` rather than forcing an inflated score.
...
5. Check duplicates before mutation.
   ...
   - When coordinates are supplied, duplicate checks compare both the stored geography point and the raw `lat`/`lng` columns so self-checks and recent coordinate updates remain stable. Duplicate responses may still include low-confidence address/region/name candidates outside the requested radius for review. Treat `GEO_OUTSIDE_REQUEST_RADIUS`, `OUTSIDE_RADIUS_REVIEW_ONLY`, or `outsideRadiusReviewOnly: true` as a caution that the candidate matched by non-radius evidence and should not be accepted as a duplicate without stronger identity evidence. If an outside-radius candidate has no strict identity evidence such as same address, same 시군구, nearby high-similarity coordinates, external refs, or provider id, it should be non-blocking `low_priority_noise` / `manual_duplicate_review`, not a create-blocking hold.
   - Duplicate responses use top-level `items` as the canonical candidate array and include `meta` counts for confidence, `suggestedAction`, `reviewBucket`, `lowPriorityNoiseCount`, and `holdReviewCount`. Do not read legacy `candidates` or `results` fields from current `/v1/places/duplicates` responses. For scripts, use shared helpers from `scripts/lib/aigo-search.ts` so missing `items` fails loudly instead of being counted as zero.
   ...
   - Duplicate candidates may include `relationshipHint` such as `same_building` or `parent_child` when the candidate is useful relationship context but lacks identity evidence. Use it as a `relatedPlaces` lead after source review; do not patch or merge the candidate solely because it appears in duplicate results.
   - Treat `BRANCH_SIBLING_REVIEW_ONLY` as a chain/brand sibling caution, not an identity duplicate. Different branches of malls, outlets, toy stores, or large retail chains can share aliases and high name similarity; do not patch one branch from another unless provider ID, external refs, exact same address, or very close coordinate/name evidence proves identity.
   - Treat `PUBLIC_PROVIDER_SIBLING_REVIEW_ONLY` as a same-provider public-facility sibling caution, not a blocking duplicate. Different branches of one 육아종합지원센터/가족센터/도서관 service, such as separate `장난감도서관` branches, can share provider and service names; use exact-name lookup and branch-specific address/source evidence before patching. If the candidate is outside the requested radius and lacks same-address, same-시군구, nearby high-similarity coordinates, external refs, or provider id, treat it as non-blocking `low_priority_noise` rather than `sibling_branch_review`.
   - Treat park/playground pairs marked `PUBLIC_SAME_SITE_SUBFACILITY_REVIEW_ONLY` as parent-child relationship context when the candidate names share a specific park-site anchor, such as a named playground inside a parent park. Do not update the parent park as if it were the playground; source-review the child facility and link it with `parent_child` or `same_site` when it is independently useful.
   - Treat `WEAK_THEMATIC_SIMILARITY_REVIEW_ONLY` and `GENERIC_ALIAS_REVIEW_ONLY` as noisy activity, alias, or subfacility similarity, not a blocking duplicate. Generic terms such as 물놀이터, 분수, 수영장, 어린이자료실, 장난감도서관, 장난감나라, 장난감대여실, and 어린이 체험실 can appear in many unrelated places; use them as a search clue only unless exact name, same address, same district, provider ID, external refs, or near-coordinate evidence proves identity.
   ...
   - Treat `UNRELATED_BRANCH_CATEGORY_REVIEW_ONLY` as same-address or nearby category noise, not an update candidate. This commonly appears when a public child facility duplicate check sees a food/restaurant branch in the same building or district; do not patch or merge it unless an external id or other strict identity source proves it is the same place.
   - Duplicate candidates include `reviewBucket` so executors can sort review workload: `identity`, `relationship_context`, `sibling_branch_review`, or `low_priority_noise`. Treat `sibling_branch_review` and `low_priority_noise` as lower-priority context unless stronger identity evidence is recorded.
...
6. Mutate through the API.
   - Use `Authorization: Bearer <AIGO_API_KEY>` for every production `/v1` API call. Do not use `x-api-key`, query-string keys, cookies, public app session headers, or other API-key header names for agent place-data work; those will be rejected by the production API and can be misdiagnosed as an invalid key.
   ...
   - For any task whose goal is registration, enrichment, dedupe verification, image-health verification, or count movement, load `AIGO_API_KEY` from the repository `.env` file before calling the API. If `.env` does not provide `AIGO_API_KEY`, stop the mutation workflow and ask the user to add or provide it; do not continue with additional research-only density sprints as a substitute for the requested API work. Read-only helper scripts that target `https://aigo.o-r.kr` also preflight `.env` / shell credentials and must fail before fetching when the key is missing, empty, or the local development default.
   - Before registration or enrichment batches, call `GET /v1/health` with `Authorization: Bearer $AIGO_API_KEY`. A `200` response means the bearer key is accepted and the API can complete a shallow database query. Treat `401` as a key/configuration issue only after confirming the request used the exact Bearer header above; a 401 from `x-api-key` or another header shape is an agent call bug, not evidence that `.env` is wrong. Treat `429` as a temporary invalid-key attempt block, and `5xx` as server or database health trouble to resolve before mutation.
   ...
   - If a production health/read-only call returns the same 401/403 credential failure as a previous heartbeat, do not keep appending identical blocker notes or notifying every run. Verify the Bearer header shape once, leave the existing blocker context intact, and end quietly until the key or API response changes.
   - Real place data work must target the deployed AiGo API at `https://aigo.o-r.kr`. Set `AIGO_API_BASE_URL=https://aigo.o-r.kr` for curl commands and helper scripts unless the user explicitly asks for disposable local route implementation testing.
   - Do not create, update, enrich, or verify real place registrations against `http://localhost:3000`. Localhost is only for API route implementation tests with throwaway data, not for source-backed place-data collection. For new place creation requested by the user, send the validated `POST /v1/places` payload only to production and verify detail/version/search against production.
   - If `AIGO_API_KEY` is not already available in the current shell/session, read it from the ignored repository `.env` file before any duplicate, search, create, update, delete, image-health, or version-history API call. Do not guess, use the local default, or proceed with unauthenticated calls.
   - Keep API keys out of tracked files, research notes, command transcripts, commit messages, and final summaries. The repository `.env` file is ignored and is the expected local source for `AIGO_API_KEY`; load it without printing the value. Do not store production keys in tracked files.
   - The default development key `change-me` is local-only. Never use it against `https://aigo.o-r.kr`. Do not use it for place registration work; only use it for disposable localhost route tests when the user explicitly asks for local implementation testing.
   ...
   - `PATCH externalRefs` is deep-merged into the existing JSON object. Send only the keys that need to be added or changed; omitted keys such as `aliases`, `koreanSearchAliases`, `coordinateProvenance`, `infoLinks`, and `reviewLinks` are preserved. Nested objects merge recursively, while arrays, scalar values, and explicit `null` replace the addressed key. Use `null` only when intentionally clearing a stored external reference field.
...
7. Verify after meaningful changes.
   ...
   - To focus image-health checks on a current batch, call `GET /v1/places/image-health?status=attention&placeIds=<id1>,<id2>` or `listPlaceImageHealth({ status: "attention", placeIds: ["<id1>", "<id2>"], limit: 100 })`. For a single-place check, `placeId=<id>` is accepted as a compatibility alias for `placeIds=<id>`; if both are present, `placeIds` is used. The helper also accepts the same comma-separated `placeIds` string as the HTTP API, and explicitly scoped `placeIds` / `placeId` lookups include `temporarily_closed` records even though the unscoped queue stays active-only.
   - When stale or broken direct image URLs are suspected, use scoped direct probing: `GET /v1/places/image-health?status=attention&placeIds=<id1>,<id2>&probeImages=true`. This is intentionally opt-in, requires 25 or fewer scoped `placeIds`, and probes with HEAD, ranged GET, then full GET fallback before treating a primary image as unreachable. A `directProbe.method` of `GET` means the image was only confirmed by the full GET fallback. Do not use direct probing for broad unscoped queue scans.
   - For search relevance, call `POST /v1/places/search` with the intended visit context and family preferences.
   - Search results include top-level `status` and, by default, include both `active` and `temporarily_closed` places so temporary closures remain searchable with status context. Send `includeStatuses: ["active"]` only when an active-only list is explicitly required.
   ...
   - Search and detail responses include top-level `structuredDataGaps` for source-backed family logistics, booking, and operating-hours fields that are still unknown on the record. Use it with `infantLogistics.missingSignals` and `openingHoursSummary`; when a listed field is confirmed by sources, patch the structured field instead of leaving the fact only in source summaries. For `openingHours`, description/text-only notes are evidence notes but do not clear the structured gap; use structured keys such as `weekly`, `periods`, `openingHoursSpecification`, `openNow`/`isOpen`, or an explicit operating status when sources support them.
   - Search and detail responses include `openingHoursSummary` so source-backed operating-hours, reservation, walk-in, session, temporary-closure, and lodging check-in/check-out evidence remains visible even when runtime scoring cannot evaluate `OPEN_NOW` from normal weekly hours. `openingHours.temporaryClosure` is first-class structured evidence and appears as `openingHoursSummary.temporaryClosure`; during the active closure period, search scoring should treat it as `CLOSED_NOW`, not `OPENING_HOURS_UNKNOWN`. Source-backed seasonal windows with `openingHours.seasonal.startDate` / `endDate`, a `weekly.regularHours` time range, period notes such as `trialPeriod` / `summerVacationPeriod`, and weekday `closureRules` are evaluated as open or closed for the searched Seoul date rather than unknown. Seasonal period maps such as `openingHours.seasonal.summer = { startMonth, endMonth, regularHours }` are also treated as structured opening-hours readiness when the period includes a time range. For accommodations, store check-in/check-out evidence under `openingHours.lodgingStayWindow.checkIn`, `openingHours.lodgingStayWindow.checkOut`, and optional `openingHours.lodgingStayWindow.sourceBacked`; it appears as `openingHoursSummary.lodgingStayWindow`. Treat it as lodging visit-window readiness, not normal public opening hours, and expect search scoring to use `LODGING_STAY_WINDOW_KNOWN` instead of `OPENING_HOURS_UNKNOWN`.
   ...
   - For duplicate-review calls that only need identity and safety signals, send `projection: "compact"` to `/v1/places/duplicates`; compact duplicate candidates keep nested `place.id/name/category/location/status`, `confidence`, `suggestedAction`, `relationshipHint`, `reviewBucket`, `distanceMeters`, `nameSimilarity`, `outsideRadiusReviewOnly`, and `reasonCodes` while omitting full detail images, sources, versions, play features, and scoring payloads.
   - When duplicate candidates include `TENANT_PARENT_REVIEW_ONLY` with `relationshipHint: "same_building"`, treat the candidate as parent/tenant context for a possible separate child record, not as an `update_existing` target. Review source evidence and model it with `relatedPlaces` if the child place is independently useful.
   - When duplicate candidates include `PUBLIC_SAME_SITE_SUBFACILITY_REVIEW_ONLY` with `reviewBucket: "relationship_context"`, treat the candidate as a same-site public subfacility relation lead, not as the same place. Broad aliases plus a shared address/site are not enough to choose an `update_existing` target unless external ids match or name+coordinate evidence is extremely strong. Use source review to decide whether the new sibling facility should be created and linked with `same_site` or `parent_child`.
   - When duplicate candidates include `PUBLIC_SUBFACILITY_REVIEW_ONLY` with `ADDRESS_REGION_CONFLICT`, no `relationshipHint`, and `reviewBucket: "low_priority_noise"`, treat the candidate as generic public-facility noise rather than a mutation blocker. Same-address or same-district public subfacility candidates can still be `relationship_context` / `parent_child` review leads.
   - When same-provider public child-care branches such as `공동육아나눔터`, `공동육아방`, or toy-library branches return `PUBLIC_PROVIDER_SIBLING_REVIEW_ONLY` with `reviewBucket: "sibling_branch_review"`, treat them as distinct sibling-branch review leads unless strict identity evidence also matches. If they are outside-radius and not same-시군구, they should normally be `low_priority_noise` context. Branch labels such as `1호점`, named branches like `별빛`, or district/place-name anchors like `용진` and `삼봉` should prevent a generic same-region alias match from becoming a hard duplicate hold on its own.
  ...
  - Recommendation distance scoring is search-intent sensitive: playground searches strongly prefer very nearby candidates, shopping-mall searches tolerate a short drive but penalize roughly hour-plus candidates, and visit/lodging searches keep two-hour-plus destinations viable. Category browsing also uses intent-specific default radius windows, and `/v1/places/search` accepts `radiusKm` up to 500 so lodging/day-trip browsing can use wider candidate pools. Pass an explicit `radiusKm` when an agent needs a narrower or wider candidate pool than the UI default.
   ...
   - Generic keyword search also checks whitespace-compacted place names and stored external aliases, so parent-facing queries with no-space accommodation or branch names can recall records that exact-name and duplicate checks already recognize.
   - For deterministic place lookup by full name, send `matchMode: "exactName"` with `query`; this restricts candidates to exact, whitespace-compacted, hyphen/dash-as-separator, `externalRefs.aliases`, or known retail-chain alias matches instead of broad keyword/tag matches. If exact-name mode receives amenity or logistics words, search meta may include `suggestedExactNameQuery`; retry that name-only query when the original exact lookup returns no useful candidate.
   - For agent/admin dedupe audits, exact-name search includes canonical temporary-closure records by default; use active-only `includeStatuses` only when deliberately excluding temporarily closed places.
   - For dense lodging areas, nearby accommodation records that only share generic terms such as kids/pool-villa/pension should be treated as manual duplicate review unless address, external id, or other strict identity evidence confirms the same place.
   ...
   - For playground searches, source-backed or inferred taxonomy activity evidence such as `outdoor_playground`, `sand_play`, and `water_play` can satisfy playground equipment confidence when `playFeatures` has not yet been split into detailed fields. Do not treat a strong public playground as equipment-unknown solely because the evidence is stored in taxonomy instead of `playFeatures`.
...
9. Keep `agent-research/` from accumulating stale raw outputs.
   ...
   - Before a large cleanup or archive pass, run `pnpm tsx scripts/summarize-agent-research-artifacts.ts --research-dir=agent-research > agent-research/compact-manifest-YYYYMMDD.md` or use `--json` for machine-readable output. The helper is read-only and summarizes file counts, raw API/result dumps, production ids, unresolved candidate lines, blocker/error lines, source URLs, image URL candidates, and coordinate-provenance lines so useful signals survive before raw artifacts are removed.
...
Create/update accepts place-name recovery aliases as top-level `aliases` and `koreanSearchAliases`. Use `aliases` for official alternate names, legacy public labels, branch variants, English/local names, and parent-facing canonical-name variants; use `koreanSearchAliases` for Korean parent search phrases or older Korean labels that should recover the same place. The API normalizes these into `externalRefs.aliases` and `externalRefs.koreanSearchAliases`, which exact-name search, keyword scoring, and duplicate checks already read. On PATCH, top-level alias fields are added to existing stored alias arrays instead of replacing them; direct `externalRefs.aliases` or `externalRefs.koreanSearchAliases` patches without top-level alias fields still follow the normal array replacement rule. When both top-level alias fields and `externalRefs` alias arrays are present, the values are de-duplicated and persisted together. Do not hide important alias intent only in research notes.
...
For partial `PATCH /v1/places/{id}` payloads, use patch mode so the validator accepts minimal source-backed updates while still checking sources, public Korean text, enum values, image provenance when images are present, unsupported nested blocks, and risky status/data-confidence changes:

```bash
pnpm tsx scripts/validate-research-payloads.ts --mode=patch <payload.json|research.md>
```

This helper checks the raw API schema plus AiGo collection rules that the schema intentionally leaves flexible: explicit `status: "active"`, `dataConfidence` limited to `agent_collected`, `official_verified`, or `user_reported`, at least one structured `images` item, workflow-approved `images[].reviewStatus: "approved"`, source-backed image provenance, Korean user-facing public text across notes, scoring, pricing, opening-hours notes, route-support notes, play-feature notes, and image descriptions, taxonomy v1 shape when supplied, and `externalRefs.coordinateProvenance.level` strong enough for mutation. It also rejects nested `facilities` and `visit` blocks because `/v1/places` create/update uses canonical top-level fields such as `indoorType`, `parkingAvailable`, `averageStayMinutes`, and `parentEffortLevel`; move those values before validating or mutating. Fix blocking lint errors before calling `POST /v1/places`.

It also warns on workflow risks that should be resolved or intentionally documented before mutation: parent-review evidence missing for family-facing categories, public text that contains personalized household assumptions, rich public destinations missing a high-yield `externalRefs.subfacilitySweep`, subfacility findings that were not reflected in structured fields or notes, and private kids cafe or indoor-playground candidates that rely only on blog/observation evidence. A structured `externalRefs.subfacilitySweep` can use `checkedAt`, `queries` or `attemptedQueries`, `findings`, and `sourceUrls`; for museums and experience facilities, child-program terms such as `어린이`, `체험`, `프로그램`, `예약`, `유모차`, `주차`, and `전망대` count as valid sweep evidence when paired with findings or source URLs. For private kids cafes, anchor branch identity with an official/operator/public-agency/public-listing source, and prefer pairing public-listing identity with operator or booking evidence such as `officialUrl`, `reservationUrl`, `official_site`, or `operator_page`.
...
- For family-outing data, prioritize images that directly show child-relevant value: children's play equipment, kids cafe interiors, indoor playground zones, child programs, stroller/baby-care facilities, family seating, safe walking/play space, water/sand/forest play, or clear child-facing signage. General scenery, parking lots, adult exercise equipment, entrances, maps, toilets, or picnic areas can be useful supporting images, but they should not crowd out child-relevant images when those are available.
- The primary image must be the most representative child/family-facing image available, not merely the prettiest or broadest place photo. For playgrounds, kids cafes, indoor play spaces, toy libraries, family restaurants, museums with child programs, and similar categories, choose a primary image that visibly communicates the actual kid activity or child facility. Use a general exterior, landscape, parking, sign, route, restroom, or map image as primary only when no citeable child-relevant image is available, and record that limitation in the image description or research notes.
...
Search taxonomy facets use the same canonical families: `familyFitGates`, `activityTypes`, `visitUseCases`, `ageBands`, `accessTags`, `logisticsTags`, and `riskTags`. Prefer `soft` mode for planning and discovery so unknown records remain eligible with `TAXONOMY_UNKNOWN`; use `required` only when the caller truly wants a hard facet gate. Use `accessTags: ["public_facility"]` for source-backed public or nonprofit family facilities such as national/municipal museums, arboretums, science centers, libraries, toy libraries, child-care support spaces, public indoor playgrounds, and government/family-center community play spaces. Do not add `public_facility` only because a place is free, educational, listed by a tourism source, located inside a public building, appears in a public safety/coordinate dataset, or is an ordinary neighborhood playground record (`primaryCategory: playground`); private tenants, commercial kids cafes, restaurants, malls, resorts, theme parks, and general local playgrounds need explicit broader public-facility/campus evidence or should stay in a hold/research queue. A public-agency source proves the source is public, not necessarily that the operator is public; require operator evidence from the facility name, official/operator source, tags, or notes before adding `public_facility`.
...
- Search filter preferences default to soft scoring; use search `preferenceMode: "required"` when the planning context needs hard gates such as indoor type, parking, nearby toilet, stroller, elevator, nursing room, diaper table, kids toilet, baby-chair, or snack/food evidence.
...
- Paid-entry details: use structured `pricing` only when source-backed. Include a compact Korean `summary`, `basisDate` or `checkedAt`, optional `staleAfterDays`, Korean item rows for child/guardian/time/reservation/free conditions, and a source URL. Do not estimate unknown prices or leave English price prose in public pricing fields.
...
- Confirm every public-facing text value changed by the task is Korean-only except for proper nouns, source titles, URLs, currency codes, and structured keys. Check notes, scoring rationale, pricing, opening-hours notes, route-support notes, play-feature notes, and image descriptions before calling the API.
