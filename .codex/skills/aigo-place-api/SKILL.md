...
7. Verify after meaningful changes.
   ...
   - Search and detail responses include `openingHoursSummary` so source-backed operating-hours, reservation, walk-in, session, temporary-closure, and lodging check-in/check-out evidence remains visible even when runtime scoring cannot evaluate `OPEN_NOW` from normal weekly hours. `openingHours.temporaryClosure` is first-class structured evidence and appears as `openingHoursSummary.temporaryClosure`; during the active closure period, search scoring should treat it as `CLOSED_NOW`, not `OPENING_HOURS_UNKNOWN`. Accommodation check-in/check-out evidence appears as `openingHoursSummary.lodgingStayWindow`; treat it as lodging visit-window readiness, not normal public opening hours, and expect search scoring to use `LODGING_STAY_WINDOW_KNOWN` instead of `OPENING_HOURS_UNKNOWN`.
